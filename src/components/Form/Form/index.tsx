...
class Form extends React.Component {
    ...
    addItem(newItem) {
        if (newItem.props.multiple) {
            ...
            this.multipleMap[newItem.uuid] = this.items.filter(
                (item) => item.props.name === newItem.props.name
            ).length;
        }
    ...
    removeItem(item) {
        if (item.props.multiple) {
            ...
            this.items
                .filter((item) => item.props.name === name)
                .forEach((item, i) => {
    ...
    save(data = this.state.data) {
        ...
        if (this.validate()) {
            invariant(action, 'No `action` provided, check the Form component you save.');
    ...
    render() {
        ...
        const classNames = cx(`${prefixCls}-form`, { [`${prefixCls}-form_${size}`]: size });
        ...
        return (
            <form onSubmit={this.handleSubmit} className={classNames} {...other}>
