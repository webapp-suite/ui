...
class Dialog extends React.Component {
    ...
    componentDidMount() {
        ...
        if (!this.dialogInstance) {
            ...
            this.dialogInstance = new ToggleNode(dialogNode, `${prefixCls}-dialog__main_open`);
        }
    ...
    backUp() {
        const dialogStacks = document.getElementsByClassName(`${prefixCls}-dialog__main`);
        const dialogStacksCount = dialogStacks.length;
        dialogStacksCount > 1 &&
            classlist(dialogStacks[dialogStacksCount - 2]).add(`${prefixCls}-dialog__main_open`);
    }
    ...
    goForward() {
        ...
        const dialogStacksCount = dialogStacks.length;
        dialogStacksCount > 0 &&
            classlist(dialogStacks[dialogStacksCount - 1]).remove(`${prefixCls}-dialog__main_open`);
    }

    close = () => {
        this.setState({ shouldHideBackdrop: true });
        const openedDialog = document.getElementsByClassName(`${prefixCls}-dialog__main_open`)[0];
        this.backUp();
