...
const scrollbarWidth = (() => {
  ...
  scrollDiv.className = 'earthui-modal__scrollbar-measure'
...
class Modal extends Component {
  ...
  componentWillUnmount () {
    if (this.containerNode) {
      ...
      classlist(body).remove('earthui-modal_open')
      if (bodyPaddingRight) {
  ...
  updateBodyState (open, prevOpen) {
    ...
    const _scrollbarWidth = body.scrollHeight > window.innerHeight ? scrollbarWidth : 0
    if (open && !prevOpen) {
      classlist(body).add('earthui-modal_open')
      body.style.paddingRight = bodyPaddingRight + _scrollbarWidth + 'px'
  ...
  renderIntoDocument () {
    ...
    const onRendered = () => {
      if (!this.toggleModal) {
        ...
        this.toggleModal = new ToggleNode(node, 'earthui-modal_open')
